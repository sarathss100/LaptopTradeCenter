...
export const productDetailPage = async ( req, res ) => {
    try {
        ...
        const discountPercentage = populatedBrands[0].products[0].discount[0].discount_percentage;
        ...
        if ( req.user ) {
            ...
            res.render('user/productDetailPage', { username, brands, user, product, discountedPrice, discountPercentage, dir } );
        } else {
            // If the user is not authenticated, render the product detail page with 'Login' as the username
            res.render( 'user/productDetailPage', { 'username': 'Login', brands, user, product, discountedPrice, discountPercentage, dir } );
        }
    } catch ( error ) {
