    ...
    try {
        ...
        for ( let i = 0; i < cart.products.length; i++ ) {
            ...
            for ( let j = 0; j < cart.products[i].productId.discount.length; j++){
                const discountPercentage = cart.products[i].productId.discount[j].discount_percentage;
        ...
        const totalOriginalPrice = originalPrice.reduce( (acc, amount) => acc = acc + amount,0);
        const totalDiscountedPrice = discountedPrice.reduce( (acc, amount) => acc += amount, 0)
        ...
        if ( req.user ) {
            ...
            res.render('user/cartPage', { username, brands, user, products, cart, originalPrice, discountedPrice, totalOriginalPrice, totalDiscountedPrice, savings  } );
        } 
    } catch ( error ) {
        ...
        if (!cart) {
            ...
            if (existingProductIndex > -1) {
                ...
                if (cart.products[existingProductIndex].quantity > 3 ) {
                    return res.status(200).json({ message: `You are added maximum number of products`});
                }
            } else {
