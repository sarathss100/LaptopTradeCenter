...
export const signUpForm = async ( req, res ) => {
   ...
   // Initialize error messages
    ...
    if ( !isValidEmail( data.email ) ) req.session.userSignupEmailFormatError = `Please enter a valid email`;
    
    if ( !isValidPhoneNumber( data.phone_number ) ) req.session.userPhoneNumberFormatError = `Please enter a valid phone number`;
    
    if ( data.password !== data.confirm_password ) req.session.userPasswordMissMatchError = `Passwords don't match!`;
    
    // If there are validation errors, redirect to the sign-up page with errors
    if ( req.session.userSignupEmailFormatError || req.session.userPhoneNumberFormatError || req.session.userPasswordMissMatchError ) {
